...
pub fn build(b: *std.Build) !void {
    ...
    if (amalgamated) {
        ...
        lib.addCSourceFiles(.{
            .root = b.path("lib/src"),
            .files = try findSourceFiles(b, neowasm),
            .flags = &.{"-std=c11"},
        });
    }
...
fn findSourceFiles(b: *std.Build, neowasm: bool) ![]const []const u8 {
    ...
    const exclude_wrong_wasm = if (neowasm) "wasm_store.c" else "wasm_store_neo.c";

    while (try iter.next()) |entry| {
        if (entry.kind != .file) continue;
        const file = entry.name;
        const ext = std.fs.path.extension(file);
        if (std.mem.eql(u8, ext, ".c") and !std.mem.eql(u8, file, "lib.c") and !std.mem.eql(u8, file, exclude_wrong_wasm)) {
