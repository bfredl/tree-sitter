...
fn cb_assert_fail(args_ret: []StackValue, in: *Instance, data: *anyopaque) !void {
    _ = args_ret;
    _ = in;
    _ = data;
    std.debug.print("assert_fail:p\n", .{});
...
extern fn iswalpha(c_int) c_int;
extern fn iswblank(c_int) c_int;
extern fn iswdigit(c_int) c_int;
extern fn iswxdigit(c_int) c_int;
extern fn iswlower(c_int) c_int;
extern fn iswupper(c_int) c_int;
extern fn towupper(c_int) c_int;
extern fn towlower(c_int) c_int;

fn cb_int_to_int(args_ret: []StackValue, in: *Instance, data: *anyopaque) !void {
    _ = in;
    const func: *const fn (c_int) c_int = @ptrCast(data);

    const cp = args_ret[0].i32;
    args_ret[0].i32 = func(cp);
}
...
fn cb_memmove(args_ret: []StackValue, in: *Instance, data: *anyopaque) !void {
    _ = data;
    const d = args_ret[0].u32();
    const s = args_ret[1].u32();
    const n = args_ret[2].u32();
    try in.memmove(d, s, n);
}

fn cb_memset(args_ret: []StackValue, in: *Instance, data: *anyopaque) !void {
    _ = data;
    const d = args_ret[0].u32();
    const val = args_ret[1].u32();
    const n = args_ret[2].u32();
    const m = in.mem.items;
...
fn wasm_load(data: []u8, langname: []u8, lexer_size: usize, any: *anyopaque) !*WASMLanguage {
    ...
    try imports.add_func("iswalnum", .{ .cb = &cb_int_to_int, .data = @constCast(@ptrCast(&iswalnum)), .n_args = 1, .n_res = 1 });
    try imports.add_func("iswalpha", .{ .cb = &cb_int_to_int, .data = @constCast(@ptrCast(&iswalpha)), .n_args = 1, .n_res = 1 });
    try imports.add_func("iswblank", .{ .cb = &cb_int_to_int, .data = @constCast(@ptrCast(&iswblank)), .n_args = 1, .n_res = 1 });
    try imports.add_func("iswdigit", .{ .cb = &cb_int_to_int, .data = @constCast(@ptrCast(&iswdigit)), .n_args = 1, .n_res = 1 });
    try imports.add_func("iswlower", .{ .cb = &cb_int_to_int, .data = @constCast(@ptrCast(&iswlower)), .n_args = 1, .n_res = 1 });
    try imports.add_func("iswspace", .{ .cb = &cb_int_to_int, .data = @constCast(@ptrCast(&iswspace)), .n_args = 1, .n_res = 1 });
    try imports.add_func("iswupper", .{ .cb = &cb_int_to_int, .data = @constCast(@ptrCast(&iswupper)), .n_args = 1, .n_res = 1 });
    try imports.add_func("iswxdigit", .{ .cb = &cb_int_to_int, .data = @constCast(@ptrCast(&iswxdigit)), .n_args = 1, .n_res = 1 });
    try imports.add_func("towupper", .{ .cb = &cb_int_to_int, .data = @constCast(@ptrCast(&towupper)), .n_args = 1, .n_res = 1 });
    try imports.add_func("towlover", .{ .cb = &cb_int_to_int, .data = @constCast(@ptrCast(&towlower)), .n_args = 1, .n_res = 1 });
    try imports.add_func("strlen", .{ .cb = &cb_strlen, .data = undefined, .n_args = 1, .n_res = 1 });
    try imports.add_func("memcmp", .{ .cb = &cb_memcmp, .data = undefined, .n_args = 3, .n_res = 1 });
    ...
    try imports.add_func("__assert_fail", .{ .cb = &cb_assert_fail, .data = undefined, .n_args = 4, .n_res = 0 });
    try imports.add_func("strncpy", .{ .cb = &cb_strncpy, .data = undefined, .n_args = 3, .n_res = 1 });
    try imports.add_func("memmove", .{ .cb = &cb_memmove, .data = undefined, .n_args = 3, .n_res = 1 });
    try imports.add_func("memset", .{ .cb = &cb_memset, .data = undefined, .n_args = 3, .n_res = 1 });
    // MISSING: memchr, strcmp, strncat, strncmp
